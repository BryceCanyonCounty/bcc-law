...
TriggerServerEvent('bcc-law:RegisterStorageSv')
...
CreateThread(function()
    while true do
        Wait(0)
        if IsControlJustReleased(0, 0x3C0A40F2) then
            ClockMenu()
        end
    end
end)
                ...
                if IsControlJustReleased(0, 0xCEFD9220) then
                    if ConfigService.CommunityServiceSettings.minigame then
                        ...
                        if test == 100 then
                            ...
                            TriggerServerEvent("bcc-law:updateservice")
                        else
...
RegisterNetEvent("bcc-law:ServicePlayer") -- Assigns Chore amount and picks random coord for construcion
AddEventHandler("bcc-law:ServicePlayer", function(chore, amount)
    Serviced = true
    Choreamount = amount
            ...
            if GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.5 and not Inmenu then
                if not isDead then
                    ...
                    if PromptHasHoldModeCompleted(Open) then
                        TriggerServerEvent("bcc-law:PlayerJob") -- run client side check before check for distance. no need to run code that is not meant for the client its optimized this way
                        Wait(200)
...
RegisterNetEvent('bcc-law:PlayerInWagon') --Put in Vehicle logic, not in use currently
AddEventHandler('bcc-law:PlayerInWagon', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('bcc-law:GetSearch')
AddEventHandler('bcc-law:GetSearch', function(obj)
    TriggerServerEvent('bcc-law:TakeFrom', obj, searchid)
end)

RegisterNetEvent("bcc-law:PlayerJob") -- Job check event
AddEventHandler("bcc-law:PlayerJob", function(data)
    print(data.job)
    PlayerJob = data.job
    PlayerJobGrade = data.grade
end)

RegisterNetEvent("bcc-law:senddata") -- Job check event
AddEventHandler("bcc-law:senddata", function(playermoney)
    Takenmoney = playermoney
end)

RegisterNetEvent("bcc-law:breakout") -- Event for breaking out
AddEventHandler("bcc-law:breakout", function()
    local coords = GetEntityCoords(PlayerPedId())
    local local_player = PlayerId()
    TriggerServerEvent('bcc-law:jailbreak')
    TriggerServerEvent('bcc-law:policenotify', coords)
    Core.NotifyBottomRight(_U('jailbreak'), 4000)
    Jailed = false
    Jail_time = 0
...
AddEventHandler("vorp:SelectedCharacter", function(charid)
    TriggerServerEvent("bcc-law:check_jail")

    if ConfigMain.HandcuffHotkeyActive then
        CreateThread(function()
            Wait(5000)
            ...
            if CheckTable(OnDutyJobs, PlayerJob) then
                while true do
                    if IsControlPressed(0, 0x8AAA0AD4) then
                        if IsDisabledControlPressed(0, 0xE30CD707) then
                            TriggerServerEvent('bcc-law:CheckHandcuffs')
                        end
                    end
                end
            else
                while true do
                    if IsControlPressed(0, 0x8AAA0AD4) then
                        if IsDisabledControlPressed(0, 0xE30CD707) then
                            Core.NotifyBottomRight(_U('youarenotonduty'), 4000)
                        end
                    end
                end
            end
...
RegisterNetEvent("bcc-law:onduty")
AddEventHandler("bcc-law:onduty", function(duty, data)
    if not duty then
        PoliceOnDuty = false
        if ConfigMain.synsociety then
            TriggerServerEvent('bcc-law:synsociety', false)
            ExecuteCommand('refreshjob')
        end
    else
        PoliceOnDuty = true
        if ConfigMain.synsociety then
            TriggerServerEvent('bcc-law:synsociety', true)
            ExecuteCommand('refreshjob')
        end
    end
end)
...
--Badges
RegisterNetEvent("bcc-law:badgeon")
AddEventHandler("bcc-law:badgeon", function()
    Wait(60)
    local ped = PlayerPedId()
    ...
    if not badgeactive then
        ...
        if PlayerJob == "police" and PlayerJobGrade ~= 6 then
            if IsPedMale(ped) then
        ...
        elseif PlayerJob == "police" and PlayerJobGrade == 6 then
            --Sheriff
            ...
            --Rangers
        elseif PlayerJob == "marshal" and PlayerJobGrade ~= nil then
            if IsPedMale(ped) then
        ...
        Core.NotifyBottomRight(_U('badgeon'), 4000)
    else
        DeleteObject(Badge)
        badgeactive = false
        Core.NotifyBottomRight(_U('badgeoff'), 4000)
    end
end)

RegisterNetEvent("bcc-law:goonduty") -- Go on duty event
AddEventHandler("bcc-law:goonduty", function()
    if PoliceOnDuty then
        Core.NotifyBottomRight(_U('onduty'), 4000)
    else
        TriggerServerEvent('bcc-law:goondutysv', GetPlayers())
    end
end)
...
AddEventHandler("bcc-law:gooffduty", function()
    TriggerServerEvent("bcc-law:gooffdutysv")
end)
...
RegisterNetEvent('bcc-law:drag') -- Event to register dragging
AddEventHandler('bcc-law:drag', function(copId)
    if IsHandcuffed then
...
RegisterNetEvent("bcc-law:JailPlayer") -- Jailing player event
AddEventHandler("bcc-law:JailPlayer", function(time, Location)
    local ped = PlayerPedId()
    ...
    if not Jailed then
        if Autotele then
            ...
            DoScreenFadeIn(500)
            Citizen.Wait(600)
            Core.NotifyBottomRight(_U('imprisoned') .. time_minutes .. _U('minutes'), 4000)
            FreezeEntityPosition(ped, false)
            TriggerEvent("bcc-law:wear_prison", ped)
            Wait(500)
        else
            Jail_time = time
            Jailed = true
...
RegisterNetEvent("bcc-law:wear_prison") -- Wear prison outfit event
AddEventHandler("bcc-law:wear_prison", function()
    local ped = PlayerPedId()
...
RegisterNetEvent("bcc-law:UnjailPlayer") -- Unjail player event
AddEventHandler("bcc-law:UnjailPlayer", function(jaillocation)
    local local_ped = PlayerPedId()
    local local_player = PlayerId()
    JailID = jaillocation
    ExecuteCommand('rc')
    Core.NotifyBottomRight(_U('released'), 4000)
    Jailed = false
    Jail_time = 0
    if Autotele then
        if JailID == "sk" then
...
RegisterNetEvent("bcc-law:cuffs") --Cuffing player event
AddEventHandler("bcc-law:cuffs", function()
    HandcuffPlayer()
end)
        ...
        if not isDead then
            ...
            if chance >= 85 then
                ...
                if breakChance < 3 then
                    TriggerServerEvent("bcc-law:lockpick:break")
                else
                    ...
                    Citizen.Wait(1250)
                    TriggerServerEvent('bcc-law:lockpicksv', GetPlayerServerId(closestPlayer))
                end
            end
        end
    else
...
CreateThread(function() -- Added time if over max distance/count down until unJailed logic
    while Jailed do
        ...
        TriggerServerEvent("bcc-law:taketime")
    end
end)

RegisterNetEvent('bcc-law:handcuff', function()
    local playerPed = PlayerPedId()
...
RegisterNetEvent('bcc-law:HandcuffsChecked')
AddEventHandler('bcc-law:HandcuffsChecked', function(hasHandcuffs)
    if hasHandcuffs then
        HandcuffPlayer()
    else
        Core.NotifyBottomRight(_U('nocuffs'), 4000)
    end
end)
RegisterNetEvent('bcc-law:lockpicked') -- Successful lockpick event
AddEventHandler('bcc-law:lockpicked', function()
    local playerPed = PlayerPedId()
    ClearPedSecondaryTask(playerPed)
    SetEnableHandcuffs(playerPed, false)
    DisablePlayerFiring(playerPed, false)
...
RegisterNetEvent("bcc-law:witness", function(coords)
    Core.NotifyLeft(_U('crimereported'), _U('jailbreakalert'), "generic_textures", "star", 6000)
